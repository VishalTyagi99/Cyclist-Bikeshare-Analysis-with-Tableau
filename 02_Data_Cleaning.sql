...
CREATE TABLE `2022_trip_data.cyclistic_cleaned_data` AS (
  SELECT 
  ride_id, 
  rideable_type,
  started_at,
  ...
  end_lng,
  member_casual,
FROM `third-harbor-397105.2022_trip_data.cyclistic_data` AS t1
JOIN 
  (SELECT ride_id,
   TIMESTAMP_DIFF(ended_at, started_at, MINUTE) AS ride_length_in_mins
  FROM `third-harbor-397105.2022_trip_data.cyclistic_data`) AS t2
USING(ride_id)
